    ...
    def _get_steps(self) -> list:
        steps = [
            Step({
                ...
                'target': 'xpath=//input[@placeholder="用户名"]',  # 使用 "username" 的定位
                'command': Command.TYPE
            }),
            Step({
                'step': 3,
                'value': self.account.secret,  # 密码
                'target': 'xpath=//input[@placeholder="密码"]',  # 使用 "password" 的定位
                'command': Command.TYPE
            }),
            Step({
                'step': 3,
                'value': None,
                'target': 'xpath=//label[contains(@class, "el-checkbox")]/span[contains(@class, "el-checkbox__input")]' ,# 定位同意使用协议的勾选框
                'command': Command.CLICK
            }),
            Step({
                'step': 4,
                'value': None,
                'target': 'xpath=//button[@class="el-button el-button--primary el-button--default is-round login-button"]',# 定位登录按钮
                'command': Command.CLICK
            })
        ]
        return steps
